...
<div class="flex flex-col lg:flex-row gap-6">
  <div id="trailer-col" class="w-full lg:w-[360px] shrink-0">
    <div class="sticky top-0 lg:top-4 z-10 bg-[#0a0a0f] pb-2">
      ...
      <div id="trailer-video" class="w-full aspect-video rounded-2xl overflow-hidden glass">
        <iframe id="yt-player" class="w-full h-full" src="https://www.youtube.com/embed/{yt_id}?autoplay=1&mute=1&rel=0" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>
      </div>
    </div>
  </div>
...
document.querySelectorAll('[data-seek]').forEach(el => {
  el.addEventListener('click', () => {
    ...
    iframe.src = iframe.src.replace(/[?&]start=\d+/,'').replace(/autoplay=1/,'autoplay=1') + '&start=' + t;
  });
});
</script>
</body></html>'''
